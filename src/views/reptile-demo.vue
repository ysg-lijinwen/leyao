...
<template>
  <div class="base-layer" v-waterMarker="{text:'乐心瑶版权所有',textColor:'rgba(180, 180, 180, 0.4)'}">
    <div class="demo-row">
      ...
      <button v-debounce="reptileGjgfkgjListDemo" style="margin-left: 10px;">国家国防科工局文章列表</button>
      <br>
      <br>
      ...
      <button v-debounce="reptileGjgfkgjBatchDemo"
        style="margin-left: 10px;">批量获取国家国防科工局文章内容列表</button>
      <br>
...
<script>
import { reptileYngfkgjContent, reptileYngfkgjList, reptileYngfkgjContentBatch, reptileGjgfkgjContent, reptileGjgfkgjList, reptileGjgfkgjContentBatch } from '@/api'
export default {
  name: "index",
  components: {},
  data () {
    return {
      targetArticleUrl: 'http://www.yngfkgj.gov.cn/NewsView.aspx?NewsID=6e1e8cb2-6590-468b-b3f6-f73d08051b64',
      targetSiteCategoryUrl: '',
      articleUrlListYngfkgj: [],
      articleUrlListGjgfkgj: [],
      startIndex: 0,
      nodeData: null,
  ...
  methods: {
    ...
    // http://www.yngfkgj.gov.cn/NewsList.aspx?ClassID=6175f5d8-6930-446d-ad24-1a563f5723d7 // 云南国防科工局新闻中心
    ...
    // http://www.sastind.gov.cn/n112/n451217/index.html // 国家国防科工局要闻栏目
    async reptileGjgfkgjListDemo () {
      if (this.targetSiteCategoryUrl && this.targetSiteCategoryUrl.startsWith(this.domainNameGjgfkgj)) {
        const res = await reptileGjgfkgjList({
          targetUrl: this.targetSiteCategoryUrl,
          domainName: this.domainNameGjgfkgj
        })
        console.log(res)
        if (res.status) {
          if (res.resData && res.resData.articleList) {
            this.articleUrlListGjgfkgj = res.resData.articleList.map((item) => {
              return item.articleUrl
            })
          }
        }
    ...
    async reptileGjgfkgjBatchDemo () {
      if (this.articleUrlListGjgfkgj && this.articleUrlListGjgfkgj.length > 0) {
        let tempList = null
        if (this.startIndex !== 0 || this.startIndex < this.articleUrlListGjgfkgj.length - 1) {
          tempList = JSON.parse(JSON.stringify(this.articleUrlListGjgfkgj.slice(this.startIndex, this.articleUrlListGjgfkgj.length)))
        } else {
          tempList = JSON.parse(JSON.stringify(this.articleUrlListGjgfkgj))
        }
        const res = await reptileGjgfkgjContentBatch({
          targetUrls: tempList,
          domainName: this.domainNameGjgfkgj
        })
        console.log(res)
